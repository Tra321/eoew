...
public class StudentDaoImpl implements StudentDao{
    ...
    @Override
    //宿舍管理
    public void chooseDormitory(String studentID,String dormitoryID,float dormitoryFloor) {
        // TODO Auto-generated method stub
        Connection con = null;
        PreparedStatement ps= null;
        ResultSet rs=null;
        try {
            con=BaseDao.getCon();
            String sql ="insert into choice (studentID,dormitoryID,dormitoryFloor) values(?,?,?)";
            ps=con.prepareStatement(sql);
    ...
    public void deleteDormitory(String studentID,String dormitoryID) {
        ...
        try {
            con=BaseDao.getCon();
            String sql="delete from choice where studentID=? and dormitoryID=? ";
            ps=con.prepareStatement(sql);
            ps.setString(1, studentID);
            ps.setString(2, dormitoryID);
            int a =ps.executeUpdate();
    ...
    public void getStuDormitory(String studentID) {
        ...
        try {
            ...
            String sql ="select * from dormitory where dormitoryID in (select dormitoryID from choice where studentID=?)";
            ps=con.prepareStatement(sql);
            ps.setString(1, studentID);
            rs=ps.executeQuery();
    ...
    public void getStuDormitoryFloor(String studentID) {
        ...
        try {
            con=BaseDao.getCon();
            String sql = "Select sum(dormitoryFloor) From dormitory,choice Where dormitory.dormitoryID=choice.dormitoryID and choice.dormitoryFloor>60 and choice.studentID=?;";
            ps=con.prepareStatement(sql);
            ps.setString(1, studentID);
            rs=ps.executeQuery();
            while (rs.next()) {
    ...
    public void getAvaDormitory(String studentID) {
        ...
        try {
            con=BaseDao.getCon();
            String sql = "select * from dormitory where dormitoryID not in (select dormitoryID from choice where studentID=?)";
            ps=con.prepareStatement(sql);
            ps.setString(1,studentID);
            rs=ps.executeQuery();
            System.out.println("dormitoryID \t dormitoryBuild \t dormitoryFloor \t");
    ...
    public void printStuDormitoryFloor(String studentID) {
        ...
        try {
            con=BaseDao.getCon();
            String sql = "select dormitoryBuild,dormitoryFloor from dormitory,choice where choice.studentID in (SELECT choice.studentID from choice,student where choice.studentID = student.studentID and student.studentID = ?);";
            ps=con.prepareStatement(sql);
            ps.setString(1,studentID);
            rs=ps.executeQuery();
            ...
            while (rs.next()) {
                Dormitory dormitory = new Dormitory();
                Live choice = new Live();
                dormitory.setDormitoryBuild(rs.getString("dormitoryBuild"));
                choice.setDormitoryFloor(rs.getInt("dormitoryFloor"));

                System.out.println(dormitory.getDormitoryBuild()+"    \t  "+choice.getDormitoryFloor());
            }
        } catch (Exception e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
